...
export const ReadList = () => {
  ...
  return (
    <div className='fixed right-0'>
      <button onClick={toggleSideBar} className='absolute top-0 -left-12 mt-5 text-2xl bg-lightbeige p-2 rounded-full shadow-2xl opacity-90 hover:opacity-100'>
        {hidden ? <ReadListIcon /> : <CloseIcon />}
      </button>
      <div className={`h-screen w-80 relative shadow-xl gap-3 p-2 bg-lightbeige bg-opacity-90 text-md overflow-auto font-semibold ${hidden && 'hidden'}`}>
